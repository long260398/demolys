                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <label id="ma_voucher-error" class="error d-none" for="ma_voucher">Không được để
                                            trống ô</label>
                                        <p class="reminder-line">Vui lòng chỉ nhập các kí tự chữ cái (a-z A-z), số
                                            (0-9);
                                            tối đa 20 kí tự.</p>
                                    </td>
                                </tr>
                                    ...
                                    <td>
                                        <div class="">
                                            <input type="number" class="inputnumber validate-input"
                                                   id="gia_tri_toi_thieu" value="" placeholder="Nhập vào">
                                            ...
                                            <div class="type-mgtd">
                                                <div class="typetd">
                                                    <input type="radio" class="changetypeaplly" name="changetypeaplly"
                                                           id="kieu_voucher_1" value="1" checked> <label
                                                            for="kieu_voucher_1">Áp dụng cho sản phẩm</label>
                                                </div>
                                                <div class="typetd">
                                                    <input type="radio" class="changetypeaplly" name="changetypeaplly"
                                                           id="kieu_voucher_2" value="2"> <label for="kieu_voucher_2">Áp
...
<script>
    $(document).ready(function () {
        ...
        $('#ma_voucher').keyup(function () {
            ...
            var regex = /^[A-Za-z0-9]{0,20}$/; //regular expression to match only alphanumeric characters and up to 5 characters
            if (!regex.test(inputVal)) {
                $(this).val(inputVal.replace(/[^A-Za-z0-9]/g, '').substring(0, 20)); //remove any non-alphanumeric characters and limit to 5 characters
            }
        });
        ...
        $("body").on("click", "#publish", function () {
            ...
            if (check == 1) {
                ...
                // Sử dụng phương thức POST và gửi từng phần một
                var pageSize = 50; // Số lượng giá trị mỗi phần
                var totalPages = Math.ceil(dataChange.length / pageSize);

                $('.divgif').css('display', 'block'); // Hiển thị loading trước khi bắt đầu gửi

                function sendData(page) {
                    var startIndex = (page - 1) * pageSize;
                    var endIndex = Math.min(startIndex + pageSize, dataChange.length);
                    var pageData = dataChange.slice(startIndex, endIndex);
                    ...
                    $.ajax({
                        ...
                        beforeSend: function () {
                            // Hiển thị loading cho mỗi phần
                            console.log('Sending part', page);
                        },
                        success: function (rs) {
                            console.log('Received response for part', page);

                            // Ẩn loading khi đã gửi xong
                            if (page === totalPages) {
